...
func (a *App) resolve(w http.ResponseWriter, r *http.Request) {
	...
	if len(errs) > 0 {
		...
		if msg, ok := errs["host"]; ok {
			errorText = msg
		} else if msg, ok := errs["cip"]; ok {
			errorText = msg
		} else if msg, ok := errs["resolve_type"]; ok {
			errorText = msg
		} else if msg, ok := errs["q"]; ok {
			errorText = msg
		}
		writeJSON(w, 400, map[string]interface{}{"ok": false, "error": errorText, "validation_errors": errs, "elapsed_ms": elapsedMs(start)})
		return
	}
...
func (a *App) buildResolvePath(host, q, cip string, signEnabled bool) (string, string, error) {
	...
	if signEnabled {
		if a.cfg.HTTPDNS.SignKey == "" {
			return "", "", errors.New("sign is enabled but sign_key is empty in config")
		}
	...
	for _, k := range keys {
		parts = append(parts, rfc3986(k)+"="+rfc3986(qv[k]))
	}
...
func (a *App) request(host, path, connectIP string) (string, error) {
	...
	if strings.TrimSpace(connectIP) != "" {
		target = strings.TrimSpace(connectIP)
	}
	u := "https://" + target + path
...
func (a *App) preloadDispatch() error {
	for i := 0; i < 3; i++ {
		if _, _, err := a.refreshDispatch("global"); err == nil {
			return nil
		}
...
func (a *App) dispatchRefreshLoop() {
	...
	for range ticker.C {
		a.dispatchMu.RLock()
		regions := make([]string, 0, len(a.dispatchPools))
		for k := range a.dispatchPools {
...
func (a *App) getDispatchSnapshot(region string) ([]Endpoint, interface{}, error) {
	...
	a.dispatchMu.RLock()
	if eps, ok := a.dispatchPools[k]; ok {
		dbg := a.dispatchMeta[k].Debug
		a.dispatchMu.RUnlock()
		return eps, dbg, nil
	}
	if eps, ok := a.dispatchPools["global"]; ok {
		dbg := a.dispatchMeta["global"].Debug
		a.dispatchMu.RUnlock()
		return eps, dbg, nil
	}
	a.dispatchMu.RUnlock()
...
func extractDispatchEndpoints(parsed map[string]interface{}, fallback string) []Endpoint {
	...
	if len(arr) == 0 {
		return []Endpoint{{Host: fallback, ConnectIP: ""}}
	}
	...
	if ds, ok := first["domains"].([]interface{}); ok {
		for _, d := range ds {
			...
			if s != "" {
				...
				out = append(out, Endpoint{Host: s})
			}
		}
	}
	if firstDomain == "" {
		firstDomain = fallback
	}
	if ips, ok := first["ips"].([]interface{}); ok {
		for _, ip := range ips {
			s := strings.TrimSpace(fmt.Sprint(ip))
			if s != "" {
				out = append(out, Endpoint{Host: firstDomain, ConnectIP: s})
			}
		}
	}
	if len(out) == 0 {
		out = append(out, Endpoint{Host: fallback})
	}
	return out
}
...
func (a *App) lookupRegion(ip string) (country, province, city, isp, raw string) {
	...
	if len(parts) >= 5 && clean(parts[1]) == "" {
		province = normP(parts[2])
		city = clean(parts[3])
		isp = normI(parts[4])
		return
	}
	if len(parts) >= 5 {
		province = normP(parts[1])
		city = clean(parts[2])
		isp = normI(parts[3])
		return
	}
...
func elapsedMs(start time.Time) int {
	return int(math.Round(float64(time.Since(start).Milliseconds())))
}
